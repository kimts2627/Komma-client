    ...
    for (let i = 0; i < soundList.length; i++) {
      if (id === soundList[i].id) {
        if (soundList[i].defaultVolume < 1) {
          modifiedSoundList[i].defaultVolume = Number(
            (modifiedSoundList[i].defaultVolume + 0.2).toFixed(1)
          );
        } else {
...
export const ListTutorial: React.FC<TutorialProps> = ({
  width,
  handleWindowSize,
}) => {
  // 원판 이용안내창 등장 후 제거
  ...
  return (
    <div className="touch-guide">
      <img
        src={touchGuide}
        alt=""
        className="pointer left touch"
        onClick={touchLeft}
      />
      <img
        src={touchGuide}
        alt=""
        className="pointer right touch"
        onClick={touchRight}
      />
    </div>
  );
