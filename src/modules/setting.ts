...
const HANDLE_USERINFO_CHANGE_MODE =
  "setting/HANDLE_USERINFO_CHANGE_MODE" as const;
...
export const handleDegree = (degree: number) => ({
  type: HANDLE_DEGREE,
  payload: { degree },
});
...
// 액션 타입
type SettingAction =
  | ReturnType<typeof handleSettingModal>
  | ReturnType<typeof handleDarkMode>
  | ReturnType<typeof handleUserInfoChangeMode>
  | ReturnType<typeof handleChangeUserInfo>
  | ReturnType<typeof getUserNameFromServer>
  | ReturnType<typeof getEmailFromServer>
  | ReturnType<typeof handleWindowSize>
  | ReturnType<typeof handleOptionPage>
  | ReturnType<typeof handleDegree>;
...
const setting = (state = initialState, action: SettingAction) => {
  switch (action.type) {
    ...
    case HANDLE_OPTION_PAGE:
      return Object.assign({}, state, {
        isGeneralOption: !state.isGeneralOption,
      });
    case HANDLE_DEGREE:
      return Object.assign({}, state, { degree: action.payload.degree });
    default:
      return state;
  }
};
