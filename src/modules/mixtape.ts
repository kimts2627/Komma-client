...
// 액션 타입
type MixtapeAction =
  | ReturnType<typeof handleDeleteMode>
  | ReturnType<typeof handleListAddModal>
  | ReturnType<typeof addItem>
  | ReturnType<typeof deleteItem>
  | ReturnType<typeof setMixtapeProperty>
  | ReturnType<typeof handleSelectedIcon>;
  ...
  switch (action.type) {
    ...
    case HANDLE_LIST_ADD_MODAL:
      return Object.assign({}, state, {
        isListAddModalOn: !state.isListAddModalOn,
      });
    case ADD_ITEM:
      return Object.assign({}, state, {
        mixtapes: [...state.mixtapes, action.payload.playLists],
      });
    case DELETE_ITEM:
      return deleteItemFromState(state, action);
    case SET_MIXTAPE_PORPERTY:
      return Object.assign({}, state, {
        mixtapes: action.payload.modifiedMixtape,
      });
...
const deleteItemFromState = (state: MixtapeState, action: any) => {
  for (let i = 0; i < state.mixtapes.length; i++) {
    if (state.mixtapes[i] === action.payload.itemId) {
      return Object.assign({}, state, {
        mixtapes: state.mixtapes.splice(i, 1),
      });
    } else {
      return state;
    }
